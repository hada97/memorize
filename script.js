...
function startGame() {
  ...
  if (shuffleMode) {
    shuffleCards(); // Se aleatório ativado embaralha as cartas
  }
...
function handleFile(event) {
  ...
  reader.onload = function (e) {
    ...
    // Lê o arquivo como ArrayBuffer, que é mais eficiente para todos os tipos de arquivo Excel
    const workbook = XLSX.read(data, { type: "array" });
    const sheet = workbook.Sheets[workbook.SheetNames[0]];

    const rows = XLSX.utils.sheet_to_json(sheet, { header: 1 }); // Lê como uma matriz
  ...
  if (currentCardElement) {
    if (showMeaning) {
      ...
      currentCardElement.textContent =
        currentCardElement.getAttribute("data-meaning"); // Exibe o significado
    } else {
...
function toggleShuffleMode() {
  ...
  if (shuffleMode) {
    ...
    document.getElementById("shuffle-cards").textContent =
      "Desativar Modo Aleatório"; // Altera o texto do botão
  } else {
    document.getElementById("shuffle-cards").textContent =
      "Ativar Modo Aleatório"; // Restaura o texto do botão
  }
}
